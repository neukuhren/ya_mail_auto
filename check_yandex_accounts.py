...
def find_label_invalid_login() -> None:
...
def check_avatar1_on_page_and_click() -> object:
...
def check_avatar3_on_page_and_click() -> object:
    """Проверяет если есть аватар пользователя, то возвращает элемент автара на странице.
    Если аватар не найден, то возвращает None
    """
    logger.debug(f'Запуск функции {inspect.currentframe().f_code.co_name}')
...
def find_magicpromopage_title_qr_code(el_class='MagicPromoPage-title'):
...
def find_advanced_captcha(el_class='AdvancedCaptcha-SilhouetteTask'):
    """Проверяет есть ли advanced каптча формата Yandex Smart Captcha 
...
def find_captcha_and_save_img(el_id: str):
    ...
    try:
        ...
        logger.debug(f'Элемент каптча {el_id} не найдена.')
...
def navigate_menu_and_change_password(el_avatar, dict_with_data) -> str:
    ...
    if image_captcha_content:
            captcha_code = decrypt_captcha_deform_text(PARAM_DICT['API_KEY_FOR_RUCAPTCHA'], file_name='image_captcha.jpg')
            if captcha_code:
...
def check_account():
    ...
    if len(dict_with_data) != 0:
        ...
        sleep(randint(1, 3))
        find_label_invalid_login()
        if find_image2fa_entertype():
            find_and_click_btn_psw()
            sleep(1)
            find_field_and_insert_passwd_text(passwd=dict_with_data['Password'])
        find_field_invalid_password()
        find_advanced_captcha()
        find_magicpromopage_title_qr_code()
        ...
        if check_entry_only_sms():
            ...
            el_avatar1 = check_avatar1_on_page_and_click()  # аватар 1 вида
...
def check_yandex_accounts_main():
    ...
    if len(dict_with_data) != 0:
        ...
        if PARAM_DICT['PARAMETER_CHANGE_PASSWORD_USE_COOKIES'] == 'True':
            ...
            if cookies is None:
                ...
                check_account_for_signin()
                # Проверяем результат авторизации
                el_avatar = check_avatar1_on_page_and_click()  # аватар 1 вида
                el_avatar2 = check_avatar2_on_page_and_click()  # аватар 2 вида
                if el_avatar or el_avatar2:
                    ...
                    try:
                        ...
                        dict_with_data['Psw_ya_for_app_email'] = el_psw.text
                        if dict_with_data['Psw_ya_for_app_email']:
                            logger.info(f'Для пользователя {dict_with_data["Login"]} получен пароль {dict_with_data["Psw_ya_for_app_email"]}')
                        ...
                        dict_with_data["Psw_ya_for_app_email"] = None
        else:
            logger.error(f'В файле config.txt установлено неверное значение параметра PARAMETER_CHANGE_PASSWORD_USE_COOKIES, установите True')
    else:
        logger.error(f'словарь dict_with_data с данными текущего аккаунта пуст')        
...
def set_password_app_main():
    ...
    Построчно читает xlsx файл и вызывает для каждого валидного аккаунта функцию получения пароля почты."""
        ...
        try: 
            ...
            while True:
                ...
                try: 
                    if len(dict_with_data) != 0:  # если словарь не пустой (был заполнен данными из файла)
                        # если статус аккаунта ok 
                        if (dict_with_data["Status"] == 'ok') and (dict_with_data["Psw_ya_for_app_email"] == None):
                            ...
                            if dict_with_data["Psw_ya_for_app_email"]:
                                cur_cell = worksheet.cell(row=current_row, column=6, value=str(dict_with_data["Psw_ya_for_app_email"]))
                    ...
                    if cur_cell.value is None:
                        ...
                        for r in range(2, current_row):
                            ...
                            c_Psw_ya_for_app_email = worksheet.cell(row=r, column=LIST_WITH_COLUMNS_COMBINED.index("Psw_ya_for_app_email")+1)
                            # если значение статуса ok не содержится в словаре со статусами исключая no status
                            if (c_status.value == 'ok') and (c_Psw_ya_for_app_email.value == None):
                                need_retry_check = True
                        if need_retry_check:
                            current_row = 2
                            logger.info(f'Начинаю повторную проверку со строки {current_row}.')

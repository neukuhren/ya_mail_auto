...
def send_messages_main(yandex_or_mail_accs: str):
    ...
    try:
        ...
        if wb_lnk and ws_lnk:  # Если xlsx книга и активный лист получены
            ...
            if yandex_or_mail_accs == 'yandex':
                file_name='combined.xlsx'
            elif yandex_or_mail_accs == 'mail':
                file_name='combined_mailru.xlsx'
        # Открываем xlsx книгу и активный лист
        wb_combined, ws_combined = open_xlsx_file_and_return_active_sheet(file_name=file_name)
        if wb_combined and ws_combined:  # Если xlsx книга и активный лист получены
            current_row = 2  # С какой строки читать xlsx файл
            logger.debug(f'Из книги {file_name} получен активный лист {ws_combined}')
        else:
            logger.critical(f'ОШИБКА! Не удалось окрыть листы книги {file_name}')
                            ...
                            try:
                                ...
                                while avatar_ is None:  # цикл до успешной авторизации
                                    ...
                                    if avatar_:  # Вход выполнен успешно
                                        ...
                                        logger.info(f'Возможно вход с аккаунта {random_login} не был успешным. Начата попытка использовать другой аккаунт...')
                                # pause = input('Нажмите клавишу Enter')
